...
Το Parking Sensor λειτουργεί σαν τους αισθητήρες παρκαρίσματος που υπάρχουν στα αυτοκίνητα. Το HC-SR04 μετρά συνεχώς την απόσταση από ένα αντικείμενο μπροστά του, ενώ ο IR sensor επιβεβαιώνει ότι υπάρχει πραγματικά κάποιο εμπόδιο. Με βάση την απόσταση, το Arduino ελέγχει τα LED, το buzzer και την οθόνη LCD.

Όταν δεν υπάρχει αντικείμενο μπροστά ή η απόσταση είναι μεγαλύτερη από περίπου 150 cm, στην οθόνη εμφανίζεται το μήνυμα SPACE AVAILABLE, ανάβει το πράσινο LED και το buzzer παραμένει σιωπηλό.

Καθώς το αντικείμενο πλησιάζει, η απόσταση εμφανίζεται στην LCD μαζί με μια μπάρα προόδου. Ταυτόχρονα το buzzer αρχίζει να παράγει διακοπτόμενα beep. Όσο μικραίνει η απόσταση, τόσο πιο γρήγορα ακούγονται τα beep, ώστε ο χρήστης να καταλαβαίνει ότι πλησιάζει στο εμπόδιο. Όταν η απόσταση πέσει κάτω από τα 50 cm, ανάβει το κόκκινο LED για επιπλέον προειδοποίηση.

Το όριο στο οποίο θα εμφανιστεί η ένδειξη STOP δεν είναι σταθερό. Καθορίζεται από το ποτενσιόμετρο, το οποίο επιτρέπει στον χρήστη να επιλέγει ζωντανά απόσταση περίπου 5–30 cm χωρίς να χρειάζεται να ξαναφορτώσει τον κώδικα στο Arduino. Η επιλεγμένη τιμή εμφανίζεται συνεχώς στη δεύτερη γραμμή της LCD ως Stop@ μαζί με την κατάσταση του IR sensor (IR:Y ή IR:N).

Όταν η μετρούμενη απόσταση γίνει μικρότερη ή ίση από το όριο που έχει οριστεί με το ποτενσιόμετρο, το σύστημα περνά σε κατάσταση STOP. Τότε εμφανίζεται στην οθόνη το μήνυμα !! STOP !!, το κόκκινο LED μένει μόνιμα αναμμένο και το buzzer παράγει συνεχή ήχο, υποδεικνύοντας ότι το όχημα πρέπει να σταματήσει.

Με αυτόν τον τρόπο ο χρήστης λαμβάνει οπτική και ηχητική ενημέρωση για την απόσταση από το εμπόδιο και μπορεί να παρκάρει με μεγαλύτερη ακρίβεια και ασφάλεια.
