...
LCD I2C      SDA -> A4   SCL -> A5 , GND -> GND , VCC -> 5v
```
...
αναλυτικα:
Το Weather Dashboard είναι ένας αυτόνομος σταθμός καιρού βασισμένος σε Arduino, ο οποίος μετρά τη θερμοκρασία με τον αισθητήρα TMP36 και τη φωτεινότητα του περιβάλλοντος με ένα LDR (φωτοαντίσταση). Οι τιμές επεξεργάζονται από το Arduino και εμφανίζονται σε οθόνη LCD 16x2. Στην πρώτη γραμμή της οθόνης εμφανίζεται η τρέχουσα θερμοκρασία μαζί με ένδειξη τάσης (↑ ↓ →) και η μέγιστη θερμοκρασία που έχει καταγραφεί, ενώ στη δεύτερη γραμμή εμφανίζεται το ποσοστό φωτεινότητας μαζί με ένα απλό bar graph.

Το σύστημα περιλαμβάνει επίσης τρία LED (πράσινο, κίτρινο και κόκκινο), τα οποία ανάβουν ανάλογα με την ένταση του φωτός στο περιβάλλον, δημιουργώντας μια οπτική ένδειξη φωτεινότητας. Δύο κουμπιά χρησιμοποιούνται για λειτουργίες ελέγχου: το ένα αλλάζει τη μονάδα μέτρησης από °C σε °F και αντίστροφα, ενώ το δεύτερο επαναφέρει τις αποθηκευμένες μέγιστες και ελάχιστες τιμές θερμοκρασίας.

Επιπλέον, το Arduino καταγράφει δεδομένα σε μορφή CSV μέσω της σειριακής θύρας κάθε λίγα δευτερόλεπτα, επιτρέποντας την εύκολη εξαγωγή και ανάλυση των μετρήσεων σε προγράμματα όπως Excel ή Google Sheets για δημιουργία γραφημάτων και παρακολούθηση τάσεων.
